class Events::Ama < ::Event
  ...
  def self.add_summaries_from_hosksaid(ama)
    ...
    description = "<p class='mb-5'>This summary was generated by <a href='https://www.hosksaid.com' target='_blank'>HoskSaid</a> with GPT-4 based on the original transcript.</p>"
    description += summary["sections"].map {|s| "<h5>#{s['topic']}</h5><p>#{s['text']}</p>" }.join
    footer_links = [
      { label: "Summary by hosksaid.com", url: summary["summary_link"] },
      { label: "Watch on YouTube", url: summary["video_url"] }
    ]
    extras = ama.extras
    extras.merge!(
      {
